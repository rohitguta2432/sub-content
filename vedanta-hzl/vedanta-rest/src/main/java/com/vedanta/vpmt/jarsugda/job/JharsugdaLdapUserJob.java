package com.vedanta.vpmt.jarsugda.job;
...
@Component
@Slf4j
public class JharsugdaLdapUserJob implements Job {
	
/*	@Value("${jharsugda.cron.expression.LDAPActiveUser}")
	...
	*/
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		log.info("STARTED-JHARSUGDA-LDAP-USER-JOB");
		
		List<Long> businessUnits = new ArrayList<>();
		businessUnits.add(Constants.JHARSUGUDA_BUSINESS_UNIT);
		...
		businessUnits.forEach( (businessUnitId)->{
			...
			(ldapAuthenticationProvider.getAllLDAPUser(businessUnitId)).forEach(activeUserResponse->{
				...
				entity.setLdapBusinessUnitId(Constants.JHARSUGUDA_BUSINESS_UNIT);
				 ldapActiveUserService.save(entity);
			});
		});
		log.info("FINISHED-JHARSUGDA-LDAP-USER-JOB");
	}
...
/*	@Bean(name = "jharsugdaLDAPActiveUserJobBean")
	public JobDetailFactoryBean LDAPActiveUserJobBean() {
		return QuartzConfig.createJobDetail(this.getClass());
	}

	@Bean(name = "jharsugdaLDAPActiveUserJobBeanTrigger")
	public CronTriggerFactoryBean LDAPActiveUserJobBeanTrigger(
			@Qualifier("jharsugdaLDAPActiveUserJobBean") JobDetailFactoryBean fetchScheduledJobBean) {
