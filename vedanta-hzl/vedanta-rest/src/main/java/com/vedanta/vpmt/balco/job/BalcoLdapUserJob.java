package com.vedanta.vpmt.balco.job;
...
@Component
@Slf4j
public class BalcoLdapUserJob implements Job {

/*	@Value("${balco.cron.expression.LDAPActiveUser}")
...
*/
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		log.info("STARTED-BALCO-LDAP-USER");

		List<Long> businessUnits = new ArrayList<>();
		businessUnits.add(Constants.BALCO_BUSINESS_UNIT);
		...
		businessUnits.forEach((businessUnitId) -> {
			...
			(ldapAuthenticationProvider.getAllLDAPUser(businessUnitId)).forEach(activeUserResponse -> {
				...
				entity.setLdapBusinessUnitId(Constants.BALCO_BUSINESS_UNIT);
				ldapActiveUserService.save(entity);
			});
		});

		log.info("FINISHED-BALCO-LDAP-USER");
...
/*	@Bean(name = "balcoLDAPActiveUserJobBean")
	public JobDetailFactoryBean LDAPActiveUserJobBean() {
		return QuartzConfig.createJobDetail(this.getClass());
	}

	@Bean(name = "balcoLDAPActiveUserJobBeanTrigger")
	public CronTriggerFactoryBean LDAPActiveUserJobBeanTrigger(
			@Qualifier("balcoLDAPActiveUserJobBean") JobDetailFactoryBean fetchScheduledJobBean) {
